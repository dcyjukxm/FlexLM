 ...
 *****************************************************************************/
/*	
 *	Module:  $Id: pnopen.c,v 1.3 2003/01/14 21:48:03 kmaclean Exp $
 * 
 *	Function:
 *
 *	Description: 
...
char *modes;		/* r/w modes of the files of interest */
{
	...
	if (t==0) {	/* child */
#if 0
		/* first, close all file descriptors other than
		 * stdin/out/err and the pipes */
		maxd = getdtablesize();
		for (d=3; d<maxd; d++) {	/* start after stderr */
			for (l=0; l<pninfo->numfiles*2; l++) {
				if (d==fd[l])
					break;	/* one of ours */
				if (d<fd[l]) {
					/* not in our table, we close it */
					(void)close(d);
					break;
				}
			}
