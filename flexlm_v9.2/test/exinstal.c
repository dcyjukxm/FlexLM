 ...
 *****************************************************************************/
/*	
 *	Module: $Id: exinstal.c,v 1.4 2003/01/13 22:46:12 kmaclean Exp $
 *
 ...
 *
 *	       	 	This example takes a file, which contains 
 *			a license in decimal or readable format.
 *			It converts the license to readable, 
 *			and checks the license-keys in the file.
 *
 *
 ...
 *				with a previous FLEXlm version, then
 *				use 
 ...
 */
	...
	{
		lc_perror(lm_job, "FLEXlm init failed");
		exit(lc_get_errno(lm_job));
	}
/*
 *	Convert the file to readable format, in case it's decimal
 */
	if (lc_convert(lm_job, buf, &readable_buf, &err, 
						LM_CONVERT_TO_READABLE))
	{
		printf("Error converting file to readable license\n");
