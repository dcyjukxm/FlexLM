 ...
 *****************************************************************************/
/*	
 *	Module: $Id: vdinfo.c,v 1.8 2003/01/13 22:46:13 kmaclean Exp $
 *
...
char **argv;
{
  ...
  int rc;
	...
	if ((rc = lc_new_job((LM_HANDLE *)0, 0, &code, &lm_job)) && 
							rc != LM_DEMOKIT)
	{
			...
			else 
			{
							...
							gi.allow_borrow);
				{
				  ...
				  char szTimeBuf[1024] = {'\0'};
					...
					ctime_r(&t1, szTimeBuf);
#else /* !NO_BUFFER_SIZE */
					ctime_r(&t1, szTimeBuf, sizeof(szTimeBuf));
#endif
					printf("%-30s%s", "lmgrd_start", szTimeBuf);
#ifdef NO_BUFFER_SIZE
					ctime_r(&t2, szTimeBuf);
#else /* !NO_BUFFER_SIZE */
					ctime_r(&t2, szTimeBuf, sizeof(szTimeBuf)));
#endif
					printf("%-30s%s", "vd_start", szTimeBuf);
#else /* !THREAD_SAFE_TIME */
					printf("%-30s%s", "lmgrd_start", ctime(&t1));
					printf("%-30s%s", "vd_start", ctime(&t2));
#endif
				}
			}
			first = 0;
