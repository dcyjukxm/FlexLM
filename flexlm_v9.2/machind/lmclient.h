 ...
 *****************************************************************************/
/* Module: $Id: lmclient.h,v 1.204.2.1 2003/07/02 23:50:40 sluu Exp $
***********************************************************/
/**   @file lmclient.h
 *    @brief Client side FLEXlm definitions
 *    @version $Revision: 1.204.2.1 $
 *
  ************************************************************/
...
#define FLEXLM_VERSION 9
#define FLEXLM_REVISION 2
#define FLEXLM_PATCH " "
#define FLEXLM_VERSION_REVISION_STRING "9.2.0"
#define FLEXLM_COPYRIGHT_YEAR "2003"
#define FLEXLM_BORROW_REVISION	1
#define COPYRIGHT_STRING(firstYear)	"Copyright (c) " #firstYear "-"FLEXLM_COPYRIGHT_YEAR" by Macrovision Corporation. All rights reserved."
...
#define LM_LAST_ERRNO			-137 /** The last valid error number */
...
#define HOSTID_FLEXLOCK 22          /* Try before you buy indicator HOSTID=FLEXLOCK */
...
#define LM_BEHAVIOR_CURRENT	LM_BEHAVIOR_V9
...
/*
 * Forces FLEXlm to include the FLEXlock static API library.
 */

#if defined (PC) && !defined(FLEXLM_DLL) && !defined(IT64)/* unavailable on Unix */
lm_extern int (*fl_global_variable_api_entry)();
lm_extern int fl_flexapi();
#define LM_USE_FLEXLOCK() fl_global_variable_api_entry = fl_flexapi;
#else
#define LM_USE_FLEXLOCK()
#endif /* PC */
...
#define LM_OPTFLAG_FLEXLOCK         0x100000  /* default is false */
#define LM_OPTFLAG_JAVA_LIC_FMT     0x200000  /* default is false */
#define LM_OPTFLAG_USE_ALL_FEATURE_LINES	0x01000000
...
#define LM_LF_FLAG_LINE_START	0x2
#define LM_LF_FLAG_CONTINUATION	0x4	/* Keep track of when last item parsed was '\\'	*/
			      char *filename;
...
#define CHECK_HOSTID_INTERVAL 30			/* at most, check
			   ...
			   char *hostname;
			   char *username;
			   struct _composit_id_info  *composite_init;
			   ...
			   jmp_buf jobcatch;	/* internal use */
			   jmp_buf tool_catch;	/* internal use */
		   	   char *pc_conninfo;	/* for future use */
		   	   char *env0;		/* internal use */
			   char *env1;		/* internal use */
			   char *env2;		/* internal use */
			   char *env3;		/* internal use */
			   char *env4;		/* internal use */
			   HOSTID *host_comp; /* internal use */
			   char *curr_registry;
#ifndef PC
			   char *rcfile;
			   time_t rcfile_t;
			   ...
			   int port_end;
			   char *key_filters;
			   LM_KEYLIST *lc_this_keylist;
			   char *l_new_job;
#ifdef PC
 			   LM_SOCKET	sockets[10];
 			   int			sockState;
#endif /* PC */
			   char *keymem;
			   char *keymem2;
...
typedef int (*lc_new_job_cb)(char *, VENDORCODE *, int, int, int, int * );
#if defined( FLEXLM_DLL) || (defined(PC) && !defined(FLEX_STATIC))
  lm_extern lc_new_job_cb L_NEW_JOB;
#if 0
#define LM_DLL_SIGN \
	int l_key_type = 1; \
	void (*l_key_gen_callback)() = 0;\
	extern void key_filter();\
	void (*l_key_callback)() = key_filter;
#endif
