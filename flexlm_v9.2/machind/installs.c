 ...
 *
 *      $Id: installs.c,v 1.20 2003/06/17 00:53:52 jwong Exp $
 *
                                ...
                                LPCTSTR Lmgrd_Path )
{
    LPCTSTR lpszBinaryPathName = Lmgrd_Path;
        ...
        if (schService == NULL)
        {
                printf("Failed to install FLEXlm License Manager:\n");
                switch (GetLastError())
                {
                ...
                case ERROR_SERVICE_EXISTS:
                        printf("FLEXlm License Manager is already installed\n");
                        break;
...
InstallServiceWin95(LPCTSTR Service_Name,LPCTSTR  License_Path,LPCTSTR Log_File_Path ,
                                LPCTSTR Lmgrd_Path )
{
  char logfile[MAX_PATH_SIZE];char commandLine[MAX_PATH_SIZE],temp[MAX_PATH_SIZE];
  HKEY hcpl;
  HKEY happ;
  DWORD dwType = 0;
  DWORD dwSize=0;
...
InstallService(LPCTSTR Service_Name,LPCTSTR  License_Path,LPCTSTR Log_File_Path ,
                                LPCTSTR Lmgrd_Path, LPCTSTR CmdLine_Params  )
{
    LPCTSTR lpszBinaryPathName = Lmgrd_Path;
    int RetOk;
        if (NT_OS==OsType)
        ...
        // Try creating/opening the registry key
         ...
         printf("\nFLEXlm License Manager is successfully installed \n"
               ...
               "\n"
               "\t* The FLEXlm License Manager will be automatically started\n"
               "\t  every time your system is booted.\n"
               "\n"
               "\t* The FLEXlm service log file is lmgrd.log in your NT system\n"
               "\t  directory.\n"
               "\n"
               "\t* To remove FLEXlm License Manager, type 'installs -r'\n");
    }
    else
         printf("The FLEXlm License Manager was not successfully \n"
...
RemoveServiceNT(LPCTSTR Service_Name)
{
    ...
    long lReturn;
                ...
                else
                {
                        ...
                        case ERROR_INVALID_NAME:
                        case ERROR_SERVICE_DOES_NOT_EXIST:
                                printf( "The FLEXlm License Manager service is already removed.\n" );
                                break;
...
BOOL remove=FALSE;
    ...
    if (argc == 1 || *argv[1] != '-') {
		printf("installs - Copyright (c) 1989-2003 by Macrovision Corporation. All rights \nreserved.\n");
        printf("\nUsage: \n");
        ...
        printf("\n");
        printf("If -n is not specified, \"FLEXlm License Manager\" is used as the service name.\n");
        printf("\n");
        ...
        printf("\n");
        printf("Refer to the FLEXlm Reference Manual for more details on the \"installs\" command.\n");
        exit(1);
    }
