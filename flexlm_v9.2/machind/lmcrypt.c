 ...
 *****************************************************************************/
/*	
 *	Module: $Id: lmcrypt.c,v 1.36 2003/05/05 15:55:55 sluu Exp $
 *
 ...
 */
		...
		else if (!strcmp(argv[i],"-maxlen"))
		{
		  int max = 0;
			i++;
			if (i >= argc) break;
			sscanf(argv[i], "%d", &max);	/* overrun threat, cure would possible be to add check where attribute is set */
 ...
 */
	...
	if (lc_cryptstr(lm_job, str, &ofile_str, /*&v*/&site_code, forceit, ifname, 
								&err))
	{
		if (efp == stderr)
		{
