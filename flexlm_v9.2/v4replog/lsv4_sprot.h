 ...
 *****************************************************************************/
/*	
 *	Module:	lsv4_sprot.h v1.5.0.0
 *
 *	v4 report.log compatibility package
 *	D. Birns
 *	4/5/96
 *
 *	Last changed:  6/17/96
 *
 */
...
int		ls_disown       lm_args((lm_noargs));
...
void 		ls_log_feature 	lm_args((int , char *, int,int, int , char * ));
void 		ls_log_head 	lm_args((lm_noargs));
void 		ls_log_open_ascii lm_args((char *));
int		ls_log_open_report  lm_args((char *, int, CLIENT_DATA *));
void 		ls_log_prefix 	lm_args((int, char *));
void 		ls_log_quote_string lm_args((FILE *, char *));
...
SERVERNUM 	ls_receive 	lm_args(( int dir, 
				int ournum, char **data, int who_flag));
int		ls_recycle_control lm_args((lm_noargs));
int		ls_release_control lm_args((lm_noargs));
...
void 		ls_serv_receive lm_args(( CLIENT_ADDR *ca, char *msgtype, 
			    char **msg));
...
int		ls_udp_send	lm_args((LM_SOCKET s, struct sockaddr_in *sock,
							 char *buf, int size));
void		ls_unlock	lm_args((int remove));
