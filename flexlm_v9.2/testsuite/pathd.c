 ...
 *****************************************************************************/
/*
 *	Module: $Id: pathd.c,v 1.37 2003/05/15 23:09:22 jwong Exp $
 *
 *	Function:  None
 *
...
VENDORCODE vendorkeys[] = {		/* Possible keys for vendor daemons */
		{ VENDORCODE_6, ENCRYPTION_SEED1^VENDOR_KEY5, ENCRYPTION_SEED2^VENDOR_KEY5,
		    VENDOR_KEY1, VENDOR_KEY2, VENDOR_KEY3, VENDOR_KEY4, 
		    FLEXLM_VERSION, FLEXLM_REVISION,
			FLEXLM_PATCH, LM_BEHAVIOR_CURRENT, 
			{CRO_KEY1, CRO_KEY2}, 
			LM_STRENGTH, LM_SIGN_LEVEL, 0
		    },
		{ VENDORCODE_6, 0xaaaaaaaa^VENDOR_KEY5, 0xbbbbbbbb^VENDOR_KEY5, VENDOR_KEY1, VENDOR_KEY2, VENDOR_KEY3, VENDOR_KEY4, 
	FLEXLM_VERSION, FLEXLM_REVISION, FLEXLM_PATCH, LM_BEHAVIOR_CURRENT, 
		{CRO_KEY1, CRO_KEY2},
			LM_STRENGTH, LM_SIGN_LEVEL, 0
			} 
		    };	/* End of vendor keys */
...
int ls_crypt_case_sensitive = 0; /* Is encryption code case-sensitive? -
					...
					encryption routines. */
#ifdef PC
#define LOCK_FILE "C:\\flexlm\\pathd.loc"
#else /* PC */
#ifdef MAC10
#define LOCK_FILE "/var/tmp/pathtestlock"
#else
#define LOCK_FILE "/usr/tmp/pathtestlock"
#endif /* MAC10 */
#endif

char *ls_user_lockfile = LOCK_FILE ; /* (char *)NULL; */
...
char *(*ls_user_lock)() = testcallback;	/* callback to allow multiple semaphore names
...
char *ls_dup_sel = LM_COUNT_DUP_STRING;  /* Duplicate selection criteria
						...
						pre-v2 clients */
int ls_use_all_feature_lines = 1; /* Use ALL copies of feature lines that are
...
int ls_compare_vendor_on_increment = 1;	/* Compare vendor-defined fields to
					 declare matches for INCREMENT lines? */
int ls_compare_vendor_on_upgrade = 1;	/* Compare vendor-defined fields to
					   declare matches for UPGRADE lines? */
char * ls_a_behavior_ver = 0;
